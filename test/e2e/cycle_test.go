...
var _ = Describe("Cycle Detection", Serial, Ordered, func() {
	var ns string
	...
	It("Direct cycle detection (Deployment -> Deployment)", func(ctx SpecContext) {
		...
		expectedError := fmt.Sprintf("direct cycle detected: adding dependency from %s creates a direct cycle: %s", obj1ID, obj1ID)
	...
	It("Indirect cycle detection (Deployment -> StatefulSet -> Deployment)", func(ctx SpecContext) {
		...
		expectedError := fmt.Sprintf("indirect cycle detected: adding dependency from %s creates a indirect cycle: %s", obj1ID, expectedCycle)
		testutils.ContainsLogs(expectedError, 1*time.Minute, 2*time.Second)
	})
})
